...
void sqlite3_connection::key(const char *pwd,int nKey)
{
	if(sqlite3_key(this->db,pwd,nKey)!=SQLITE_OK)
		throw database_error("unable to key database");
}

void sqlite3_connection::rekey(const char *pwd,int nKey)
{
	if(sqlite3_rekey(this->db,pwd,nKey)!=SQLITE_OK)
		throw database_error("unable to key database");
}
